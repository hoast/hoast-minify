...
module.exports = function(options) {
	...
	const mod = async function(hoast, files) {
		...
		files.forEach(function(file) {
			...
			if (hoast.helpers.matchExpressions(file.path, this.expressionsCSS, options.patternOptions.all)) {
				file.content.data = minifyCSS(file.content.data);
				debug(`File content minified as CSS.`);
				return;
			}
			if (hoast.helpers.matchExpressions(file.path, this.expressionsHTML, options.patternOptions.all)) {
				file.content.data = minifyHTML(file.content.data);
				debug(`File content minified as HTML.`);
				return;
			}
			if (hoast.helpers.matchExpressions(file.path, this.expressionsJS, options.patternOptions.all)) {
	...
	mod.before = function(hoast) {
		...
		if (options.patternsCSS) {
			this.expressionsCSS = hoast.helpers.parsePatterns(options.patternsCSS, options.patternOptions, true);
			debug(`CSS patterns parsed into expressions: ${this.expressionsCSS}.`);
		}
		if (options.patternsHTML) {
			this.expressionsHTML = hoast.helpers.parsePatterns(options.patternsHTML, options.patternOptions, true);
			debug(`HTML patterns parsed into expressions: ${this.expressionsHTML}.`);
		}
		if (options.patternsJS) {
			this.expressionsJS = hoast.helpers.parsePatterns(options.patternsJS, options.patternOptions, true);
